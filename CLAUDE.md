...
- **Persistence**: `data.json` is the only store — global settings + two docid-keyed maps: the pinned set and per-node overrides (`nodeOverrides`; so renames are non-events). **Nothing is per-document**; every SETTING is global — an override is a global fact about a doc, like a pin. Saving an override is a write intent (lazy `ensureDocId` + the pin eligibility/refusal seam in `PersistenceServices`); the orphan sweep prunes both maps. Every persisted shape carries a `version` field.
