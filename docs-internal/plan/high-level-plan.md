...
- **We prefer not to own canvas parsing.** Adaptive strategy: at build time, detect whether the install's `resolvedLinks` contains `.canvas` keys. If yes, canvas edges flow through the same code path as markdown and our parser never runs. If no, a fallback provider parses `.canvas` JSON.
