...
- **Both settings surfaces write through ONE pipeline** (`view/settingsWritePipeline.ts`, one instance per plugin), which fixes what users actually saw (ticket `nid_m5hxe4eo9jgt7cfic7s2o3uvi_e`). The guarantees it states:
    - **Nothing a control shows lags the input.** A stepper, slider, toggle or typed field moves on the interaction; the graph rebuild that carries the value back is allowed to take as long as it takes. Precisely: the control keeps showing what the user last asked for while the store still holds the value the burst STARTED from, or one of the burst's own earlier values (its writes echo back one at a time — flickering through them would be worse than lagging). The moment the store shows anything else — another surface wrote, or the write path clamped what was typed — the store wins, so a control can never claim a value that was not stored.
