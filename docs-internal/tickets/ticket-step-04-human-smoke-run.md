...
2. Run command **"Vicinity Graph: Open vicinity graph"** → view opens in the **right sidebar**. Open `note1.md` → expect the vicinity rendered as plain React Flow nodes (note1/note2/note3 + test.canvas), pan/zoom/fit-view working.
3. **Navigation**: click through note1 → note2 → note3 → the graph rebuilds to each active note's vicinity. Opening a non-eligible file (e.g. `pic.png`) leaves the graph unchanged (MAIN gating — md/canvas only).
...
7. **View lifecycle**: close/reopen the view; drag the view from the sidebar into the main editor area and back (draggable view type); open a **second** vicinity-graph view at the same time → both track their own active file independently, no cross-contamination.
...
- Steps 1–4 (open in right sidebar, render vicinity, navigation, click-to-open): ✅ as expected (implied by graph rendering + updating on edit).
