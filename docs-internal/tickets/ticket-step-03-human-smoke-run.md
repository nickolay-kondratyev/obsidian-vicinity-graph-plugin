...
2. Open `note1.md` → run "Vicinity Graph: Debug: log vicinity graph for active file" → expect nodes note1/note2/note3 + test.canvas, attachment `pic.png` as first image.
...
3. Leave the vault open ~15s after plugin load → confirm the console shows `vicinity-graph: orphan sweep complete ...` (the completion log, with per-category removed counts), delayed + chunked, no errors.
...
- **Sweep (step 3):** ✅ `vicinity-graph: orphan sweep complete docDataFilesRemoved=[0] pinsRemoved=[0] centralEntriesRemoved=[0] ownersRewritten=[0]` — delayed, no errors.
