...
/**
 * STRONG BLOCK (ticket `nid_tvtm9gj5zaj4tbfbpti3v6sy2_e`, human DECISION): "OFF means zero
 * network" is only enforceable if the seam is the SOLE place a NETWORK-BEARING external host
 * or a network call can appear. This scan fails the build if any module OUTSIDE the
 * sanctioned seam names an owned external host literal, a `requestUrl(`, or a `fetch(`. Same
 * repo idiom as `typedNumberFields.test.ts` / the `ACCESSOR_OWNED_SYMBOLS` scan: the
 * both-states unit test (`ExternalContentUrls.test.ts`) proves the seam refuses when OFF;
 * this proves nobody else can route around it.
 *
 * WHY these three and NOT a blanket `http(s)://` scan: the guarantee is about NETWORK, and
 * only the owned hosts (`i.ytimg.com` poster CDN, `www.youtube-nocookie.com` embed) plus
 * `fetch`/`requestUrl` cause a request. A bare `https://` literal does NOT — a canonical
 * watch link (`https://www.youtube.com/watch?v=<id>`, built next to its videoId in
 * `YoutubeHeroEmbed.ts`) is an IDENTITY string a user clicks, no automatic network. The
 * human spec named exactly "external host literal, fetch(, or requestUrl("; scanning every
 * URL literal was an over-reach that conflated identity strings with calls and produced a
 * false build failure. So the leak surface guarded here is precisely the network-bearing
 * set — nothing wider.
 *
 * WHY a source scan and not a type: "no other module reaches an external host / makes a
 * call" is a claim about every FILE in the bundle, including ones no test renders. A type
 * can force the gate on the seam's own API; only a scan can prove the API is the only door.
 */

const SRC_DIR = join(dirname(fileURLToPath(import.meta.url)), "..");

/**
 * The sanctioned seam — the ONLY non-test source allowed to name the owned network hosts /
 * network calls. `ExternalContentUrls.ts` is the URL builder; the future `requestUrl`/`fetch`
...
/**
 * Source with COMMENTS removed, so a host or `fetch()` mentioned in prose (the libavoid
...
describe("external-content seam: it is the only door to the network", () => {
	it("WHEN any non-seam module is scanned THEN it names no owned external host, fetch or requestUrl", () => {
