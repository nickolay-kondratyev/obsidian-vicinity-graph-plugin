 ...
 * eligible files, then drops exactly the orphans — pins and per-node overrides
 * whose docid no longer resolves. The scan itself is NOT this class's knowledge:
...
export class OrphanSweeper {
	...
	async run(): Promise<SweepSummary> {
		...
		if (!scan.everyFileRead) {
			...
			// sweeps again.
			return { pinsRemoved: 0, overridesRemoved: 0, everyFileRead: false };
		}
	...
	private async apply(plan: SweepPlan): Promise<SweepSummary> {
		...
		return { pinsRemoved: pins.length, overridesRemoved: overrides.length, everyFileRead: true };
	}
