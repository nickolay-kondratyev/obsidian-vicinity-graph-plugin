 ...
 * drilling through RF is impossible. Provided by `NeighborhoodGraphFlow`.
 */
export const GraphUiContext = createContext<GraphUiPort | null>(null);

/** The graph's UI services; throws when rendered outside `NeighborhoodGraphFlow` (programmer error). */
export function useGraphUi(): GraphUiPort {
	const ui = useContext(GraphUiContext);
	if (ui === null) {
		throw new Error("GraphUiContext is missing — node components must render inside NeighborhoodGraphFlow");
	}
	return ui;
}
