 ...
 * every `![[…]]` written there is escaped to render as its own raw text (ticket
 * `nid_0dle910iia37t42t28dqndc5b_e`), because the drawer renders snippets
 * through Obsidian's markdown renderer, which would otherwise expand each embed
 * into the whole embedded note. Escaping happens HERE, not in the pure
...
function occurrenceAt(text: string, offset: number): LinkOccurrence {
	...
	return {
		...
		context: {
			...snippet,
			shortContext: MarkdownEmbeds.flattened(snippet.shortContext),
			expandedContext: MarkdownEmbeds.flattened(snippet.expandedContext),
		},
	};
}
