...
Goal: add the "Edge depth into groups" slider (per-endpoint depth allowance N, default 0 = current behavior) and make rendered edges reach up to N group-boundary levels below the pair's LCA container instead of always projecting onto the LCA's direct child. RENDER-ONLY: src/view/elkMapping.ts attachEdgesToContainers keeps the depth-0 projection unchanged.
...
3. Projection: parametrize the endpoint projection in src/view/folderGrouping.ts (today projectOntoContainerChildOf projects to depth 0 below the LCA's direct child) with a depth allowance; an endpoint deeper than N projects to its ancestor group at depth N. src/view/flowMapping.ts buildFlowEdges consumes the allowance from settings; aggregation (CollapsedEdgeAccumulator keying, count badge, bidirectional merge, notePairs flyout) works unchanged on the deeper projected pair.
