...
FAILING TEST (committed, it.fails — flip to it as acceptance): src/persistence/PathDocIdMap.test.ts, "WHEN a docid was seen at TWO live paths THEN deleting one twin yields no cleanup key".
...
The committed it.fails test (or its relocated equivalent) flips to it and passes; deleting a copied note never erases the surviving original's pins/overrides.
