...
1. FIRST vault write in the codebase: src/adapters/obsidianPorts.ts VaultPort is read-only today. Add a separate small write seam (OCP - new interface, do not widen VaultPort), e.g. `NoteCreationPort { create(path, content): Promise<TFile-ish> }`, implemented over `app.vault.create`, plus an open-in-editor capability (workspace leaf openFile) - likely on GraphUiPort or a sibling port; keep it ONE cohesive adapter class (suggest src/adapters/ChildNoteCreator.ts) owning: resolve target folder for a folder-note path (pure logic belongs in src/shared/FolderNotes.ts - add e.g. `ownedFolderOf(notePath)` if not already derivable), dedupe untitled name against existing files, create, open. Pure name/folder logic gets BDD unit tests with Fake ports.
...
## Testing (repo gates)
- BDD unit tests: folder-note -> target folder resolution (both patterns + conflict), untitled dedupe, refusal/no-chip when main is not a folder note.
- Component test (jsdom, settingsPanelHarness-style pattern is for the panel; for NoteNode follow existing NoteNode/component test conventions if present, else source-level tests): chip renders only for main+folder-note, has aria-label "Create child note".
...
## Acceptance criteria
- Chip appears ONLY on hover of the MAIN node when it is a folder note (both patterns); never on non-main or non-folder-note nodes.
